<template>
  <el-table :data="tableData" :row-style="showRow" v-bind="$attrs" v-on="$listeners" class='_table' header-cell-class-name="_table-head"  height="680">
    <slot name="selection" />
...
export default {
  ...
  methods: {
    ...
    showSperadIcon(record) {
      return record['childs'] && record['childs'].length > 0
    },
    toggleExpanded(trIndex) {
      const record = this.tableData[trIndex]
      const expand = !record._expand
      record._expand = expand
      if(!this.tableData[trIndex].children) this.$emit('trIndex',this.tableData[trIndex],trIndex)
