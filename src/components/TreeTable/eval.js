...
export function addAttrs(data, { parent = null, preIndex = false, X_index = false, level = 0, expand = false, children = 'children', show = true, select = false } = {}) {
  data.forEach((item, index) => {
    const _id = (preIndex ? `${preIndex}-${index}` : index) + '';
    const _index = (X_index ? `${X_index}-${index+1}` : index+1) + '';
    Vue.set(item, '_id', _id)
    Vue.set(item, '_indexXH', _index)
